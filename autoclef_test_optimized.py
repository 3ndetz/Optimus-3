...
# CAMERA_SCALER default from VPT 360.0 / 2400.0 = 0.15
# But in java side we just applying delta to person 360 rotation, so calculations is another
DEFAULT_CAMERA_SENS = 1.5
DEFAULT_ACTION_TASK = "Mine the oak log block"  # "mine_block:oak_log"

# 1. It goest to model.reset then to model._generate in agent that converts task name to local VLA needs...


#"chop tree" (simple task in natural lang) OR
# "mine_block:oak_log" (VPT-STYLE)
# "dig down and mine 11 cobblestone" (from plan example generated by planning expert from their model)
# other like "mine 1 oak_log"
...
class MineBridgeApp:
    def __init__(self, nickname, server, port=DEFAULT_PORT, web_port=WEB_PORT):
        ...
        self.last_action = {}
        self.last_keys = {}
        # Optimus-3\src\minecraftoptimus\utils\mc_slot.py
        # Optimus-3\src\minecraftoptimus\model\steve1\VPT\run_inverse_dynamics_model.py
        # GOT FROM 360.0 / 2400.0 = 0.15
        # like here Optimus-3\MineStudio\minestudio\simulator\entry.py
                ...
                while self.main_thread_active["active"] and mc.handshake():
                    ...
                    if True:
                        if self.task_type == "captioning":
                            ...
                            self.pprint("Model embodied_qa result: " + str(model.answer(task, img_action)))
                            # Embodied QA returns text, not action; just skip action
                            action = {"camera": [0.0, 0.0]}
                        elif self.task_type == "grounding":
                            self.pprint("Model grounding result: " + str(model.grounding(task, img_action)))
                        ...
                        if True:  # always do action
                            if self.task_type == "action":
                                raw_action, memory = model.get_action(
                                    input={"image": obs},
                                    task=self.text,
                                    # deterministic=True
                                    # f it freezes or does nothing in Deterministic mode, it confirms it has no idea what to do and was just flailing before.
                                )
